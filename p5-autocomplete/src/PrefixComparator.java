    ...
    public int compare(Term v, Term w) {
        ...
        if (vString.length() < myPrefixSize || wString.length() < myPrefixSize) {
            if (vString.length() <= wString.length()) {
                ...
                count = wString.length();
            }
        }

        for (int i = 0; i < count; i++) {
            if (vString.charAt(i) < wString.charAt(i)) {
                return -1;
            } else if (vString.charAt(i) > wString.charAt(i)) {
                return 1;
            }
        }
        ...
        if (vString.length() < myPrefixSize || wString.length() < myPrefixSize) {
            if (vString.length() < wString.length()) {
                return -1;
            } else if (vString.length() > wString.length()) {
                return 1;
            }
        }
